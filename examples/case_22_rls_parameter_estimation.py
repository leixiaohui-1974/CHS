...
def run_rls_estimation_simulation():
    """
    Defines and runs a simulation to test the RecursiveLeastSquaresAgent.
    """
    ...
    rls_config = {
        ...
        "components": {
            ...
            "estimator": {
                ...
                "properties": {
                    "initial_params": {"a1": 0.1, "b1": 0.1}, # Initial guesses
                    "forgetting_factor": 0.98
                }
            }
        },
        "connections": [], # Execution order handles the data flow
        "execution_order": [
            # 1. Get the next inflow value from our signal generator
            {
                "component": "inflow_signal",
                "method": "step",
                "args": {"t": "simulation.t", "dt": "simulation.dt"}
            },
            # 2. Feed the inflow to the plant to get the outflow
            # The plant's output is calculated based on the *previous* step's inflow
            {
                "component": "plant",
                "method": "step",
                "args": {"inflow": "inflow_signal.output"},
            },
            # 3. Feed the new inflow and resulting outflow to the estimator
            # The estimator will then update its parameter estimates
            {
                "component": "estimator",
                "method": "step",
                "args": {
    ...
    manager = SimulationManager(config=rls_config)
...
def plot_results(df, true_a1, true_b1):
    ...
    if not os.path.exists('results'):
        os.makedirs('results')
    save_path = 'results/case_22_rls_parameter_estimation.png'
    plt.savefig(save_path)
    print(f"Plot saved to {save_path}")
    # plt.show()

if __name__ == "__main__":
    results, t_a1, t_b1 = run_rls_estimation_simulation()
