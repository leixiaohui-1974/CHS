...
from .config_parser import parse_topology, parse_disturbances
...
class SimulationManager:
    ...
    1. By providing a `config` dictionary with the full simulation setup.
    2. By providing a `case_path` to a directory containing configuration files
       (`topology.yml`, `disturbances.csv`, `control_parameters.yaml`).

    The dictionary-based approach is preferred for SDK usage.
    """
    def __init__(self, config: dict = None, case_path: str = None):
        ...
        Args:
            config (dict, optional): A dictionary containing the entire simulation configuration.
                ...
                - 'topology': A dictionary representing the system structure.
                - 'disturbances': A list of dictionaries for time-series data.
                - 'control_params': A dictionary for controller parameters.
    ...
    def _load_from_case_path(self):
        """Loads all necessary configuration files from a case path and populates the config."""
