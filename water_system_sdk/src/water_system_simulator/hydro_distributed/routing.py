...
def identify_river_network(fac: np.ndarray, threshold: int) -> np.ndarray:
    """
    ...
    """
    return np.asarray(fac > threshold, dtype=bool)
...
class HillslopeRouting:
    ...
    """
    def __init__(self, dem: np.ndarray, fdr: np.ndarray, cell_size: float = 30.0) -> None:
        """
        Initializes the hillslope routing model.

        Args:
            dem: The elevation grid (m).
            fdr: The flow direction grid (D8).
            cell_size: The size of a grid cell in meters.
        """
        self.dem: np.ndarray = dem
        self.fdr: np.ndarray = fdr
        self.cell_size: float = cell_size
        self.travel_time_grid: np.ndarray = np.zeros_like(dem, dtype=float)

        # Map D8 codes to (dr, dc, distance_multiplier)
        self.d8_info: Dict[int, Tuple[int, int, float]] = {
            1: (0, 1, 1.0), 2: (1, 1, 1.414), 4: (1, 0, 1.0), 8: (1, -1, 1.414),
            16: (0, -1, 1.0), 32: (-1, -1, 1.414), 64: (-1, 0, 1.0), 128: (-1, 1, 1.414)
        }

    def calculate_travel_times(self, channel_network: np.ndarray, velocity_coefficient: float = 0.5) -> np.ndarray:
        """
        ...
        Args:
            channel_network: Boolean grid where True is a channel.
            velocity_coefficient: A coefficient for the velocity calculation (v = C * sqrt(slope)).
        """
        ...
        upstream_neighbors: List[List[List[Tuple[int, int]]]] = [[[] for _ in range(cols)] for _ in range(rows)]
        for r in range(rows):
            for c in range(cols):
...
class ChannelRouting:
    ...
    """
    def __init__(self, fdr: np.ndarray, channel_network: np.ndarray, k: float, x: float, dt: float) -> None:
        """
        Initializes the channel routing model.

        Args:
            fdr: The flow direction grid.
            channel_network: The boolean channel network grid.
            k: Muskingum storage time constant (units should match dt).
            x: Muskingum weighting factor (0 to 0.5).
            dt: Time step duration.
        """
        self.fdr: np.ndarray = fdr
        self.channel_network: np.ndarray = channel_network
        self.k: float = k
        self.x: float = x
        self.dt: float = dt

        # Pre-calculate Muskingum coefficients
        denominator: float = self.k * (1 - self.x) + 0.5 * self.dt
        if denominator == 0:
            raise ValueError("Muskingum denominator is zero. Check K, x, and dt.")
        self.c1: float = (0.5 * self.dt - self.k * self.x) / denominator
        self.c2: float = (0.5 * self.dt + self.k * self.x) / denominator
        self.c3: float = (self.k * (1 - self.x) - 0.5 * self.dt) / denominator

        # State variables: Inflow and Outflow for each channel cell from the previous step
        self.prev_inflow: np.ndarray = np.zeros_like(fdr, dtype=float)
        self.prev_outflow: np.ndarray = np.zeros_like(fdr, dtype=float)

        # Get channel cells in upstream-to-downstream order
        self.channel_cells_ordered: List[Tuple[int, int]] = self._topological_sort(fdr, channel_network)

        self.d8_to_offset: Dict[int, Tuple[int, int]] = {
            1: (0, 1), 2: (1, 1), 4: (1, 0), 8: (1, -1),
            16: (0, -1), 32: (-1, -1), 64: (-1, 0), 128: (-1, 1)
        }

    def _topological_sort(self, fdr: np.ndarray, channel_network: np.ndarray) -> List[Tuple[int, int]]:
        """Topologically sorts the channel network for routing."""
        rows, cols = fdr.shape
        in_degree: np.ndarray = np.zeros_like(fdr, dtype=int)

        channel_indices: np.ndarray = np.argwhere(channel_network)
        ...
        queue: deque[Tuple[int, int]] = deque([(r, c) for r, c in channel_indices if in_degree[r, c] == 0])
        ordered_list: List[Tuple[int, int]] = []
        while queue:
            r, c = queue.popleft()
    ...
    def route_flows(self, lateral_inflows: np.ndarray) -> np.ndarray:
        """
        Routes water through the channel network for one time step.

        Args:
            lateral_inflows: Grid of lateral inflows for the current step.

        Returns:
            Grid of outflows from each channel cell for the current step.
        """
        current_inflow: np.ndarray = np.zeros_like(self.fdr, dtype=float)
        current_outflow: np.ndarray = np.zeros_like(self.fdr, dtype=float)
        ...
        for r, c in self.channel_cells_ordered:
            ...
            outflow: float = self.c1 * current_inflow[r, c] + self.c2 * self.prev_inflow[r, c] + self.c3 * self.prev_outflow[r, c]
